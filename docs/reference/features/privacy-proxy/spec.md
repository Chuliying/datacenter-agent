...
| v0.3.1 | 2026-07-13 | 併入紅隊 surface-2（tag-exfil）重跑結論：C3 補強（連續 index 零知識枚舉、單一共享 token、無 rate limit → 整表可 dump、tool-args 第二通道、audit 偵測盲區、restore 文法勿收 `[]`）；新增 M9（TTL index 回收 → 還原錯人）。 |
...
| C3 | **restore 是 global、無狀態、無認證的解密 oracle**：index 為 per-(scope,kind) 連續（MAX+1），零知識枚舉 1..N；單一共享 bearer token（session_id 不綁 privacy scope）、`history` 未限長、無 rate limit → **整張對照表可在數個請求內解密 dump**，比 DB 竊取更糟（磁碟有 AES，restore 直接吐明文）。第二通道：模型可在 MCP tool args 植入 `⟦BANK_1⟧`（chokepoint 2 會還原）。§7.5 counts-only audit 結構性偵測不到（未追「本回合發出的 tag」） | restore **只還原本回合 mask 實際發出的 tag**（per-turn issued-set，或 tag 內嵌 per-turn HMAC nonce `⟦PERSON_1.<nonce>⟧`），含 tool-args 還原點；store 改 **session/tenant scope**（非 global）；index 用隨機非連續；restore 採嚴格 canonical 文法（**勿再收 `[]`/【】**——否則合法答案含 `[PERSON_1]` 也被誤還原，無需攻擊者）；mask 前中和內文 tag 形狀字串 |
...
| M9 | **TTL index 回收**：index 過期 purge 後被重新指派給不同實體（MAX+1）→ 舊報表中的 `⟦PERSON_1⟧` 之後還原成**別人**（破 NFR-P02 一致性 + 洩漏） | index 不回收（單調遞增或 128-bit 隨機 token id） |
