...
| Contract | Runtime（預設） | Legacy（明確 false/0 rollback） |
|---|---|---|
| prompt cap | config，EV pack 為 4000 chars | 2000 chars |
| `/agent` structural error | HTTP 400 | HTTP 400 |
| `/agent/stream` structural error | HTTP 200 SSE `error` frame | HTTP 400 before stream |
| REST intent | Final resolved；Refused/Aborted unknown | `unknown` |
| `intent.resolved` | yes，answer token 前 | no |
| memory/audit/policy | partial runtime components | no runtime components |

`RUNTIME_ENABLED=false/0` 會在 runtime config 載入前跳過組裝，因此能力包損壞時仍可啟動 legacy rollback path。
